...
namespace Tools.Lib.MenuContext
{
    internal class UwpHelper
    {
        ...
        public static string GetPackageName(string uwpName)
        {
            ...
            using (RegistryKey packageKey = RegistryExtension.GetRegistryKey(PackageRegPath))
            {
                if (packageKey == null) 
                    return null;
        ...
        public static string GetRegPath(string uwpName, Guid guid)
        {
            ...
            if (packageName == null) 
                ...
                return $@"{PackageRegPath}\{packageName}\Class\{guid:B}";
        }
        ...
        public static string GetFilePath(string uwpName, Guid guid)
        {
            ...
            if (regPath == null) 
                return null;
            string packageName = GetPackageName(uwpName);
            using (RegistryKey pKey = RegistryExtension.GetRegistryKey($@"{PackagesRegPath}\{packageName}"))
            {
                if (pKey == null) 
                    return null;
