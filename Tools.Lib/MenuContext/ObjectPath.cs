       ...
       // public enum PathType { File, Directory, Registry }
        ...
        public static string ExtractFilePath(string command)
        {
            if (string.IsNullOrWhiteSpace(command)) 
                return null;
            if (FilePathDic.ContainsKey(command)) 
                return FilePathDic[command];
            else
            {
        ...
        public static bool ObjectPathExist(string path, PathType type)
        {
            switch (type)
            {
                ...
                case PathType.Registry:
                    return RegistryExtension.GetRegistryKey(path) != null;
                default:
                    return false;
            }
        }
        ...
        {
            string newPath;
            string dirPath = type == PathType.Registry ? RegistryExtension.GetParentPath(oldPath) : Path.GetDirectoryName(oldPath);
            string name = type == PathType.Registry ? RegistryExtension.GetKeyName(oldPath) : Path.GetFileNameWithoutExtension(oldPath);
